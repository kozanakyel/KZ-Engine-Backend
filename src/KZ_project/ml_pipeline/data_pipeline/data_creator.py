...
from KZ_project.ml_pipeline.japanese_candlestick.japanese_candlestick_creator import (
    JapaneseCandlestickCreator,
)
...
class DataCreator(IBinaryFeatureLabel):
    ...
    def download_ohlc_from_client(self):
        if self.period:  # Only yahoo download check this data got or not
            df_download = self.client.get_history(
                self.symbol, self.interval, None, None, self.period
            )
        else:
            df_download = self.client.get_history(
                symbol=self.symbol,
    ...
    def reindex_and_sorted_cols(self, dframe):
        ...
        dframe = dframe.reindex(
            sorted(dframe.columns), axis=1
        )  # sort Columns with to alphabetic order
    ...
    def get_candlesticks(self, is_complete=False):
        ...
        if is_complete:
            ...
            df = df.tail(20).copy()  # directly default value
...
if __name__ == "__main__":
    from KZ_project.Infrastructure.services.yahoo_service.yahoo_client import (
        YahooClient,
    )
