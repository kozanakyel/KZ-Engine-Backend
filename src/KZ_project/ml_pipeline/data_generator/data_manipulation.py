# OLD VERSION WITHOUT OBJECT RELATIONS BUT WOKRS CORRECTLY IF YOU WANT TO USE
...
if __name__ == '__main__':
    ...
    # indicator_df_result = FactoryIndicatorBuilder.create_indicators_columns(d.df, d.range_list, logger=d.logger) 
            #indicators = Indicators(self.df, self.range_list, logger=self.logger)    # Create Indicator class fro calculating tech. indicators
            #indicators.create_indicators_columns()        
            #self.df = indicators.df.copy()
    # d.df = indicator_df_result
    # fe = FeatureExtractor(d.df, d.range_list, d.interval, d.logger)
    # fe.create_featured_matrix()
    # fm_data = fe.featured_matrix
    # print(fm_data)
    
    agg_matrix = FeaturedMatrixPipeline(d, f)
