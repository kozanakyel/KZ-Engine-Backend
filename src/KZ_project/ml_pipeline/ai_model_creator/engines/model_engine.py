...
class ModelEngine(IFeeCalculateable, IReturnDataCreatable):
    ...
    def save_model_to_service(self, xtest, acc_score, x_shape):
        ...
        res_str = services.save_crypto_forecast_model_service(acc_score, get_session(), self.symbol_cut, 
                                                          self.symbol, self.source, x_shape.shape[1], self.model_name,
                                                          self.interval, self.ai_type,
                                                          datetime_t)
...
if __name__ == '__main__':
    ...
    forecaster  = XgboostBinaryForecaster(n_estimators=7000, eta=0.3, max_depth=1, early_stopping_rounds=0, cv=5, is_kfold=True)
    model_engine = ModelEngine(data_creator.symbol, 'btc', data_creator.source, data_creator.interval, forecaster, is_backtest=True)
