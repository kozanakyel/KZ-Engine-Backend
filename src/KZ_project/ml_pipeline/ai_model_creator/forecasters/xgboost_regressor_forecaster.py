...
class XgboostRegressorForecaster(AbstractForecaster):
    ...
    def fit(self):
        self._model.fit(
            ...
            eval_set=self.evalset,
        )
        if self.cv > 0 and self.is_kfold:
            kfold = KFold(n_splits=self.cv, shuffle=True)
            self.kf_cv_scores = cross_val_score(
                self._model, self.X_train, self.y_train, cv=kfold
            )
            self.log(f"Kfold CV did with {self.cv} fold")
        elif self.cv > 0:
            self.cv_scores = cross_val_score(
                self._model, self.X_train, self.y_train, cv=self.cv
            )
    ...
    def get_score(self) -> float:
        ...
        if self.is_kfold:
            ...
            self.log(
                f"Score for Mean {self.cv} Cross-Validation is: {self.cv_scores.mean()}"
            )
    ...
    def get_n_importance_features(self, n: int):
        ...
        dict_importance = {
            col_list[i]: self._model.feature_importances_[i]
            for i in range(len(col_list))
        }
        sorted_d = dict(
            sorted(dict_importance.items(), key=operator.itemgetter(1), reverse=True)
        )
