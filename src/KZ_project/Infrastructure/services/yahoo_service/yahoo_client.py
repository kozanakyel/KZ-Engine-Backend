...
class YahooClient(IClientService):
    ...
    def get_history(self, symbol, interval, start, end=None, *args):
        if args:  # Only yahoo download check this data got or not
            ...
            df_download = self.yahoo_download(
    ...
    def yahoo_download(
        ...
        if period != None:
            ...
            self.log(
                f"Get {symbol} data from yahoo period: {period} and interval: {interval}"
            )
        elif start != None:
            download_dfy = yf.download(symbol, start=start, end=end, interval=interval)
            self.log(
                f"Get {symbol} data from yahoo start date: {start} and {end} also interval: {interval}"
            )
        else:
