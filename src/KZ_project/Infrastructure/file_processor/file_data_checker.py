import os
from KZ_project.Infrastructure.file_processor.folder_data_checker import (
    FolderDataChecker,
)
...
class FileDataChecker(FolderDataChecker):
    ...
    def create_pure(self) -> str:
        if self.period != None:
            ...
            self.pure_file = (
                f"{self.symbol}_{self.start_date}_{self.end_date}_{self.interval}.csv"
            )
    ...
    def create_feature(self) -> str:
        if self.period != None:
            self.feature_file = (
                f"{self.symbol}_df_feature_{self.period}_{self.interval}.csv"
            )
        else:
