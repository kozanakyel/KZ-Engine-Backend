...
def get_signal_tracker_with_ticker(ticker: str, repo: AbstractBaseRepository, session):
...
def prediction_service_new_signaltracker(
    ai_type,
    Xt,
    ...
    try:
        ...
        result_fm = get_forecast_model(symbol, interval, ai_type, repo_fm, session)
        add_signal_tracker(
            next_candle_prediction,
            hashtag,
            tweet_counts,
...
def save_crypto_forecast_model_service(
    accuracy_score,
    session,
    ticker,
    symbol,
    ...
    repo_cr = CryptoRepository(session)
    try:
        finding_crypto = get_crypto(ticker=ticker, repo=repo_cr, session=session)
...
def add_sentiment_record_from_dataframe(df_sent: DataFrame, session) -> None:
